...
function SearchInput(){
    ...
    const {displayCards, isLoading, isError, setIsLoading, setIsError} = UseFetch(`https://api.edamam.com/search?q=${query}&app_id=${YOUR_APP_ID}&app_key=${YOUR_APP_KEY}`);
        ...
        {isLoading ? <Loader /> :
            <section className={classes.infoContainer}>
                { displayCards && displayCards.length === 0 ? <h1 className={classes.displayTxt}>No result Available</h1> : (
                    ...
                    return <SearchCard key={pos} cardImg={item.recipe.image} cardCalories={wholeNumber(item.recipe.calories)} cardName={truncateStr(item.recipe.label)} style={{backgroundColor: bgStyle[Math.floor(Math.random() * bgStyle.length)]}}/>}) )
                }
            </section>
        }
        </>
